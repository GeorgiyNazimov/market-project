...
oauth2_scheme = OAuth2PasswordBearer(tokenUrl=f"{get_settings().PATH_PREFIX}/auth/token")
...
async def authenticate_user(authData: OAuth2PasswordRequestForm, session: AsyncSession) -> User:
...
async def get_current_user(token: str = Security(oauth2_scheme), session: AsyncSession = Depends(get_session)) -> User:
