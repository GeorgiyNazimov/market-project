...
class UserAdmin(ModelView, model=User):
    ...
    column_filters = [
        StaticValuesFilter(
            column=User.role, values=[("user", "user"), ("admin", "admin")]
        )
    ]
    page_size = 50
    can_delete = False
...
class CartAdmin(ModelView, model=Cart):
    ...
    column_filters = [OperationColumnFilter(Cart.total_items)]
    page_size = 20
...
admin_views = [UserAdmin, ProductAdmin, CartAdmin, CartItemAdmin]
