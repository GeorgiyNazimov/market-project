...
@pytest.mark.asyncio
async def test_api_delete_order_success(
...
@pytest.mark.asyncio
async def test_api_delete_order_unauthorized_error(
    db_session, async_client, app, override_get_session
):
    user = user_factory()
    product = product_factory(stock=5)
...
@pytest.mark.asyncio
async def test_api_delete_order_foreign_user_id_error(
    db_session, async_client, app, override_get_session
):
    my_user = user_factory()
    other_order = order_factory()
    product = product_factory(stock=5)
    other_order_item = order_item_factory(
        order=other_order, product=product, quantity=5
    )
    db_session.add_all(
        [
            my_user,
...
@pytest.mark.asyncio
async def test_api_delete_order_admin_access_success(
    db_session, async_client, app, override_get_session
):
