...
@pytest.mark.asyncio
async def test_get_product_list_without_cursor(async_client, db_session, override_get_session, app):
...
@pytest.mark.asyncio
async def test_get_product_list_with_cursor(async_client, db_session, override_get_session, app):
    products = multiple_products_factory(5)
    products.sort(key=lambda x: (x.created_at, x.id), reverse=True)
    db_session.add_all(products)
    await db_session.flush()
    ...
    response = await async_client.get(f"/api/v1/product/?limit=2&created_at_cursor={created_at_cursor}&id_cursor={id_cursor}")
