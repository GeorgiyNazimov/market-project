...
async def test_get_product_reviews_without_cursor(db_session):
    ...
    reviews, _ = await get_product_reviews_list_db(new_product.id, None, None, 2, db_session)
...
async def test_get_product_reviews_with_cursor(db_session):
    ...
    db_session.add_all(new_reviews)
    await db_session.flush()
    _, next_cursor = await get_product_reviews_list_db(new_product.id, None, None, 2, db_session)
    
    reviews, _ = await get_product_reviews_list_db(new_product.id, next_cursor["created_at"], next_cursor["id"], 2, db_session)
