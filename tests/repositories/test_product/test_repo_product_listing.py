...
async def test_get_product_list_without_cursor(db_session):
    ...
    product_list, _ = await get_product_list_from_db(None, None, limit=2, session=db_session)
...
async def test_get_product_list_with_cursor(db_session):
    ...
    await db_session.flush()
    new_products.sort(key=lambda x: (x.created_at, x.id), reverse=True)
    _, next_cursor = await get_product_list_from_db(None, None, limit=2, session=db_session)

    product_list, next_cursor = await get_product_list_from_db(next_cursor["created_at"], next_cursor["id"], 2, db_session)
